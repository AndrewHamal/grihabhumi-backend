@php
    ...
    $previewURL =
        str_replace(
            '--slug--',
            $value,
            url(url(env('APP_FRONTEND')) . $prefix) . '/' . config('packages.slug.general.pattern'),
        ) .
        $endingURL .
        (Auth::user() && $preview ? '?preview=true' : '');
@endphp
...
<div id="edit-slug-box" @if (empty($value) && !$errors->has($name)) class="hidden" @endif>
    @if (in_array(Route::currentRouteName(), ['pages.create', 'pages.edit']) &&
            BaseHelper::isHomepage(Route::current()->parameter('page')))
    ...
    @else
        <label class="control-label @if ($editable) required @endif"
            for="current-slug">{{ trans('core/base::forms.permalink') }}:</label>
        <span id="sample-permalink" class="d-inline-block" dir="ltr">
            <a class="permalink" target="_blank" href="{{ url(env('APP_FRONTEND') . $prefix . '/' . $value) }}">
                <span class="default-slug">{{ url(env('APP_FRONTEND') . $prefix) }}/<span
        ...
        @if ($editable)
            <span id="edit-slug-buttons">
                ...
                <button type="button" class="save btn btn-secondary"
                    id="btn-ok">{{ trans('core/base::forms.ok') }}</button>

...
class ConsultController extends BaseController
{
    ...
    public function ajaxGetConsult(Request $request)
    {
        $propertyId = \DB::table('re_properties')
        ->where('author_id', auth()->id())
        ->pluck('id')->toArray();

        $consults = $this->consultRepository
        ->getModel()->query();

        $consults->whereIn('property_id',  $propertyId);

        if(!empty($request->keyword)){
            $keyword = $request->keyword;
            $consults->where(function($query) use ($keyword){
    ...
    public function ajaxChangeConsult($id='', Request $request, BaseHttpResponse $response)
    {
        $consults = $this->consultRepository
        ->getModel()
        ->find($id);
        ...
        return $response->setMessage('Status Changed Successfully!');
    }

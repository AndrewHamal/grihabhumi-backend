...
class PublicController extends Controller
{
    ...
    public function getServices($id='', Request $request)
    {
        if($id)
        {
            return Career::whereHas('slugable', function($query) use ($id) {
        ...
        {
            $services = Career::with('slugable')
            ->get()
            ->transform(function($row) {
                $mapData = collect();
                $lists = json_decode($row->lists)[0];
                if($row->lists){
                    $finalData;

                    foreach(json_decode($row->lists) as $key=>$lists) {
                        $finalData[] = [
                            'services' => $row->name,
                            $lists[0]->key => $lists[0]->value,
                            $lists[1]->key => $lists[1]->value,
                            $lists[2]->key => $lists[2]->value,
                            $lists[3]->key => $lists[3]->value,
                            $lists[4]->key => $lists[4]->value,
                            ($lists[5]->key ?? 'phone') => $lists[5]->value ?? '',
                        ];
                    }
            ...
            $mergedArray = array_filter(array_merge(...$services));
            $mergedArray = collect(array_values($mergedArray));
            ...
            ->map(function($row){
                ...
                if($row->banner)
                {
                    foreach(json_decode($row->banner) as $banner){
                        $banners[] = $banner;
                    }
                }
                return $banners;
            ...
            $mergedBanner = array_filter(array_merge(...$banners));
            $mergedBanner = collect(array_values($mergedBanner));

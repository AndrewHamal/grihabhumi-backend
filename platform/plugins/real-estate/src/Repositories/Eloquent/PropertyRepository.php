...
class PropertyRepository extends RepositoriesAbstract implements PropertyInterface
{
     ...
     * {@inheritdoc}
     */
    public function getProperties($filters = [], $params = [], $per_page=6)
    {
        $filters = array_merge([
            'keyword'     => null,
            'type'        => null,
        ...
        if($filters['premium']) {
            $this->model = $this->model->where('re_properties.is_premium', $filters['premium']);
        }

        if($filters['featured']) {
            $this->model = $this->model->where('re_properties.is_featured', $filters['featured']);
        }

        if($filters['availability']) {
            $availability = array_filter(explode(',', $filters['availability']));
            $this->model = $this->model->whereIn('re_properties.availability', $availability);
        }
        ...
        if ($filters['category_id'] !== null) {
            ...
            $categoryIds = array_filter(explode(',', $filters['category_id']));
        ...
        if($filters['property_type'] !== null){
            $propertyTypes = array_filter(explode(',', $filters['property_type']));
            $this->model = $this->model
            ->whereIn('property_type', $propertyTypes);
        }

        if($filters['furnishing'] !== null){
            $furnishing = array_filter(explode(',', $filters['furnishing']));

            $this->model = $this->model
            ->whereIn('furnishing', $furnishing);
        }

        if($filters['house_type'] !== null){
            $houseType = array_filter(explode(',', $filters['house_type']));

            $this->model = $this->model
            ->whereIn('house_type', $houseType);
        }

        if ($filters['city_id']) {
            $city_id = array_filter(explode(',', $filters['city_id']));
            $this->model = $this->model->where('re_properties.city_id', $city_id);
        ...
        if(auth()->check()){
            $this->model = $this->model->where('author_id', auth()->id());
        ...
        if($filters['condition']) {
            $condition = array_filter(explode(',', $filters['condition']));
            $this->model = $this->model->whereIn('re_properties.condition', $condition);
        }

        if($filters['negotiable']) {
            $this->model = $this->model->where('re_properties.negotiable', $filters['negotiable']);
        }
    ...
    public function getPropertyBySlug(string $propertySlug)
    {
        if(is_numeric($propertySlug)){
            $property = $this->model->with('features', 'facilities')->where('id', $propertySlug)
            ->first();
        } else {
            $property = $this->model->with('features', 'facilities')->whereHas('slugable', function($query) use ($propertySlug){
                $query->where('key', $propertySlug);
            })->first();
        }
        ...
        $currentProperty = $this->findById($property->id, ['categories']);
        ...
        $property->related_properties = $this->advancedGet($params);

...
class ProjectRepository extends RepositoriesAbstract implements ProjectInterface
{
    ...
    public function getProjects($filters = [], $params = [])
    {
        ...
        $this->model = $this->originalModel->with('investor:id,name', 'categories:id,name');
    ...
    public function getProjectBySlug($id){
        $project = $this->model->with('features', 'facilities', 'property', 'investor')->whereHas('slugable', function($query) use ($id){
            $query->where('key', $id);
        })->first();

        $project->related_properties = $this->getRelatedProjects($project->id, 4, ['investor', 'slugable']);
        return $project;
    }
